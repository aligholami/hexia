...
class VQA_SAN:
    ...
    def get_data(self):
        ...
        question = tf.map_fn(lambda x: tf.string_split([x]).values, question, dtype=tf.string)
        answer = tf.map_fn(lambda x: tf.string_split([x]).values, answer, dtype=tf.string)
